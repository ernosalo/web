...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const checkTickerOverflow = () => {
      ...
      // scrollWidth gives the full content width even if clipped
      const listWidth = list.scrollWidth;
      // If list width is greater than container, we need to scroll
      setIsTickerScrolling(listWidth >= containerWidth - 20); 
    };

    // Observer for both container and content changes
    ...
    // Check after a short delay to account for layout shifts
    ...
    <div className="flex h-screen w-full bg-slate-950 overflow-hidden text-slate-100 relative">
      ...
      {!sidebarOpen && isGameRunning && (
        <button 
          ...
          className="fixed bottom-6 left-6 z-[300] w-12 h-12 bg-emerald-600 hover:bg-emerald-500 rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(16,185,129,0.4)] border border-emerald-400 transition-all hover:scale-110 active:scale-95 group"
        >
              ...
              <div className="pt-4 border-t border-slate-800">
                ...
                {pointsToEarn >= 1 && (
                  <div className="mt-4 p-3 bg-rose-900/20 rounded-xl border border-rose-500/30 animate-in fade-in zoom-in duration-500">
                    ...
                    <button onClick={() => setActiveTab('rebirth')} className="w-full py-2 bg-rose-600 hover:bg-rose-500 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] text-white">Collapse Timeline</button>
                  </div>
                )}
              </div>
            </div>
            {showSidebarScrollIndicator && (
              <div className="absolute bottom-4 left-0 right-0 flex justify-center pointer-events-none z-20 animate-in fade-in duration-300">
                <div className="flex flex-col items-center gap-1 animate-bounce text-emerald-400">
                  <svg className="w-3.5 h-3.5 drop-shadow-[0_0_12px_rgba(16,185,129,1)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </div>
              </div>
            )}
          </div>
      ...
      <main className={`flex-1 flex flex-col transition-all duration-300 ${sidebarOpen && isGameRunning ? 'lg:pl-80' : 'pl-0'} relative overflow-hidden`}>
        <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-900/40 backdrop-blur-xl shrink-0 z-[110]">
          <div className="flex items-center gap-4 shrink-0">
            ...
            {showLegacyIndicator && (
              <button 
                ...
                className={`flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 px-3 py-1.5 rounded-xl shadow-[0_0_15px_rgba(244,63,94,0.2)] animate-in slide-in-from-left duration-500 transition-all hover:bg-rose-500/20 active:scale-95 ${!isTickerVisible ? 'border-rose-500/20 grayscale-[0.2]' : 'border-rose-500/50 shadow-[0_0_20px_rgba(244,63,94,0.4)]'}`}
              >
                ...
                <div className="flex flex-col leading-none text-left">
                  ...
                  <span className="text-[10px] font-black text-rose-400 uppercase">{totalLegacyUpgrades} Upgrades Active</span>
                </div>
              </button>
            )}
            <div className="h-4 w-[1px] bg-slate-800" />
            <div className="flex items-center gap-3 h-8 min-w-[120px]">
              {isSaving && (
                <div className="flex items-center gap-2 animate-in fade-in duration-300">
                  <div className="w-2.5 h-2.5 rounded-full bg-amber-500 shadow-[0_0_12px_rgba(245,158,11,0.8)] animate-pulse" />
                  <span className="text-[10px] font-mono uppercase font-black tracking-[0.15em] whitespace-nowrap text-amber-500">
                    AUTOSAVING...
                  </span>
                </div>
              )}
          ...
          <div className="flex items-center gap-2 bg-slate-800/50 p-2 rounded-2xl border border-slate-700 shadow-lg shrink-0">
            {([1, 10, 25, 100, 'next', 'max'] as const).map(q => (
              <button key={String(q)} onClick={() => setBuyQuantity(q)} className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all ${buyQuantity === q ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>{q === 'max' ? 'Max' : q === 'next' ? 'Next' : `${q}x`}</button>
            ))}
          </div>
        </header>

        {!sidebarOpen && (
          <div className="h-16 bg-slate-950/40 border-b border-slate-800/50 flex items-center px-10 overflow-x-auto no-scrollbar shrink-0 z-[105] animate-in slide-in-from-top duration-300">
              ...
              {activeResources.map(res => (
                <div key={res} className="flex items-center gap-2.5 bg-slate-900/40 px-3 py-1.5 rounded-2xl border border-slate-800/60 hover:border-emerald-500/30 transition-colors">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className={`text-[11px] font-mono font-bold ${resourceRates[res] >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
        ...
        {gameState.rebirthUpgradeIds.length > 0 && isTickerVisible && (
          <div className="h-12 bg-slate-900/80 border-b border-rose-500/20 flex items-center overflow-hidden shrink-0 z-[104] group relative backdrop-blur-md">
            <div className="flex items-center gap-3 px-10 border-r border-rose-500/20 bg-slate-950 h-full z-20">
            ...
            <div ref={tickerContainerRef} className="flex-1 overflow-hidden relative h-full flex items-center">
               <div 
                 className={`whitespace-nowrap flex items-center gap-12 py-2 ${isTickerScrolling ? 'animate-marquee' : 'px-8'}`}
               >
                  ...
                  {/* Seamless loop clones - only render if we are actually scrolling */}
                  {isTickerScrolling && (
               ...
               {/* Vignette/Fade edges to make the scrolling look smoother */}
               {isTickerScrolling && (
                 <>
                   <div className="absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-slate-950/40 to-transparent z-10 pointer-events-none" />
                   <div className="absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-slate-950/40 to-transparent z-10 pointer-events-none" />
                 </>
               )}
            </div>
          </div>
        ...
        <div className="relative flex-1 overflow-hidden flex flex-col">
          ...
          <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-10 no-scrollbar relative scroll-smooth">
            <div className="pt-2" />
            
            {activeTab === 'buildings' && (
              <div className="grid gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 pb-32">
      ...
      {deleteRequestSlot !== null && (
        <DeleteConfirmation 
          ...
          onConfirm={() => actions.handleDeleteSlot(deleteRequestSlot)} 
